...
| 13 | **A pass probe asserts that the code still exists and still goes through the mechanism** | Rule 4 only says "it compiled". A pass probe stays green with its body emptied (measured on 8 of 10). Put a type-level assertion (`const _: fn(A) -> B = f;`, or a trait bound) **at the call site** — inside a macro's output it disappears with the macro (measured). **When the return type cannot be named** — an RPITIT `impl Future`, for instance — the full `const _: fn(A) -> B = f;` is unavailable, but *existence* still is: `const _: () = { let _ = f::<T>; };` gated on the same feature as the probe. Measured in #48: without it, re-pointing a probe's `#[cfg]` at another **declared** feature left the row green with the function never compiled, and a Cargo feature-dependency restructure did **not** close that (also measured). **Remaining limit**: emptying a function body is still not catchable at the type level — for that the probe needs a runtime assertion on an effect only its body produces |
