...
## 8. A derive that builds inside the defining module is a forgery route

> Placed at the end so §3–§7 keep their numbers: this document is referenced by
> section from the specs, the knowledge banks and `CLAUDE.md`, and a renumber
> whose grep misses one leaves a citation pointing silently at the wrong rule.
> Read it as a peer of §1 and §2, not as a lesser rule than §7.

**The principle.** A derive expands *at the item's definition site*. So an impl
generated by `#[derive(..)]` on a type with private fields assembles that type
from **inside its own module**, where privacy does not apply — and if the impl is
a trait the caller can invoke, the derive has handed construction to anyone who
can **name the type**. Field privacy buys nothing against this. The guarantee
rests entirely on the type being unnameable.

This is the general form. It is stated here rather than as a list of derives,
because a list is both a duplicate and an under-statement: the enumeration had
been copied into **four documents across seven sites**, the general form itself
into **three**, while the next derive someone adds is on none of them.

> Counted when #35 reduced them, and the counting is the point. **Three of the ten
> sites were invisible to the search that found the other seven**: one lives in
> `docs/dev/`, which is `.gitignore`d and therefore skipped by a recursive `grep`
> from the repository root; two were *paraphrases* that the enumeration's own
> wording does not match. Grep the claim (`forging route`), not the formatting —
> a duplicate that has been reworded does not look like the thing you copied it
> from.

### The two measured instances

Both are foreign-crate, no `unsafe`, and neither names a single field of the
target. `spikes/domain-opacity-sqlx/` is the reproduction.

| Probe | Derive | The forgery |
|---|---|---|
| **P15** | `sqlx::FromRow` | `query_as::<_, PubRepr>("SELECT 1 AS id, 'attacker' AS name, …")` against the attacker's **own** in-memory sqlite. No table, no access to the application's database |
| **P41** | `serde::Deserialize` | `serde_json::from_str::<PubRepr>(r#"{"id":1,"name":"attacker",…}"#)`. Strictly cheaper — `FromRow` at least needs a live connection to hand the row over; this needs a string literal |

Both live in `separate-repo/tests/forge.rs` and are `pass` rows: **the forgery
succeeds.** P41 was added by #35 — until then `Deserialize` was asserted in prose
across **three** documents (`proc-macro.md`, `persistence.md`, and
`unverified-boundaries.md`) and had never been compiled, so "both measured" was
not yet a true sentence.

### Field privacy is the wrong lever, and that was measured too

P11 and P12 reject a struct **literal** with `E0451`, and that was originally
read as "`Repr` public with private fields cannot be forged". It is a different
question. A literal is written at the *call site*; a derived impl is written at
the *definition site*. P15 exists to record the correction, and it is worth
keeping in mind as the shape of the mistake — the rejection was real, it just did
not answer what it was taken to answer.

### What closes it: the name, not the fields

Because the lever is the type's name, the response is to make the name
unreachable. `#[domain]` emits the `Repr` into a macro-owned child module **with
no visibility modifier** — not `pub`, not `pub(crate)`, and not `pub(super)`
([ADR-0010](../adr/0010-domain-constructor-confined-by-module-privacy.md),
[ADR-0011](../adr/0011-domain-is-an-attribute-macro.md)). Then there is no path
to write, so there is no derived impl to call: `E0603`, measured as **P30** and,
path-qualified, **P39d / P39e**.

So under the shipped shape **neither instance above is reachable**, and the
ledger records paths 3 and 4 as confined on exactly that condition. This section
is not a report of a live hole. It is what stops a future shape change from
re-opening one — the moment a `Repr` becomes nameable for any reason, every
derive on it is back in scope, including ones added for unrelated convenience.

> **The consequence for `repr_derive(..)`.** The attribute passes user-named
> derives through onto the generated `Repr`, with no allow-list. That is
> consistent with this section — the `Repr` is unnameable, so a spliced
> `Deserialize` has no caller — but it means the pass-through has **no second
> line of defence**. It is listed here so the dependency is explicit rather than
> discovered later.
...
- ❌ Expose an escape hatch without a proof token — recording it would be
  forgotten.
- ❌ Rely on **private fields** to stop a type being constructed when a derive on
  that type can construct it from inside the module (§8). Make the name
  unreachable instead.
