...
# The Domain constructor is confined to a macro-owned module, not to the user's

> **⚠️ Corrected by [ADR-0011](./0011-domain-is-an-attribute-macro.md).** This
> record said "derive-owned" and "what the derive emits" throughout. **A derive
> cannot emit the shape below** — the re-export collides with the user's own item
> (`E0255`, probe P38). The decision stands unchanged; the macro form that
> implements it is `#[domain]`, an attribute. Read "derive" below as "the Domain
> macro".
...
* **E. Emit the constructor, the `Repr` and the repository into a **macro-owned
...
* **Bad, and ARK-002 is not satisfied.** `rustc --explain E0624` names both
  ...
  unavailable to the user — the module is macro-owned — but the error still emits
...
| Probe | Fails if… |
|---|---|
| **P31** (`E0624`) | the constructor leaves the macro-owned module — mutation-verified: `pub(crate)` turns it red |
...
* Good, because it **does** produce `E0277` carrying Verum's
  ...
  error[E0277]: `NoProof` cannot authorise constructing a Domain value
    = note: Only the Repository generated by `#[domain]` may build this
