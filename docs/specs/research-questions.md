...
| **`Ctx<'req, E>` × async** | Whether the combination with RPITIT and async closures holds | If it does not, the spawn route cannot be closed in types. **The measurement is complete (T-M1-02 / #14; `spikes/ctx-lifetime-rpitit/` reproduces it in 19 probes), but folding the verdict into the specs is waiting on #38** |
