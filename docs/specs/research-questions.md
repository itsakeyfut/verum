...
|---|---|---|
| ~~**Domain opacity × sqlx**~~ | **Settled (T-M1-01 / #13, compile-verified)**: the sqlx interoperation holds. "The trust boundary is the repository implementation" **does not** (`Repr` is reachable from anywhere in the same crate — ledger path 21). And the derive the specification names cannot generate the required shape. Detail in [`persistence.md`](./persistence.md) §Verdict; reproduction in `spikes/domain-opacity-sqlx/`. **#33 / [ADR-0010](../adr/0010-domain-constructor-confined-by-module-privacy.md) settles the closure**: emitting the `Repr`, the constructor and the repository into a derive-owned private module closes path 21 for all user-written code — conditional on the conversion staying an inherent method. | — |
