...
"mutates": {
  "enforcement": {
    ...
    "voided_by": ["domain_repr", "repository_impl", "unscanned_effect", "..."]
  }
}
```
...
**Decided 2026-08-15. ⚠️ REOPENED 2026-08-16 by T-M1-07 (#37). Semantics settled
2026-08-18 by #42 / [ADR-0014](../adr/0014-syntactically-present-replaces-observed.md).**
The premise this decision rests on was measured and **does not hold as written**.
The rejected options are at the end.

> ### What the first version of this decision got wrong
>
> Recorded so the next reader does not re-derive it. All four were in the text as
> written on 2026-08-15, and each is now measured:
>
> 1. **"Generation gives the lower bound."** It does not. Token presence is neither
>    a subset nor a superset of what runs — a `#[cfg]`-gated effect appears though it
>    never compiles (probe V2), and six ordinary constructs are missed (#37). The
>    field is renamed `syntactically_present` and **there is no lower bound**.
> 2. **"`observed` names what happens."** Nothing observed anything; a proc macro
>    read tokens. The name went the way of `type_checked` and
>    `scope_of_readonly_guarantee` — removed, not annotated.
> 3. **"The difference detects over-declaration."** Not for **dead code**, which both
>    sides count: `if false { … }` still has to satisfy its `Has` bound (`E0277`,
>    probe D2) and is still scanned (probe D1), so the difference is empty for it.
>    The gate is a **warning** now, and §When over-declaration appears says what it
>    does catch.
> 4. **"The scan covers `handle`."** It covers calls spelled `ctx.<accessor>()` in
>    **one item** — hence `scope: "ctx_spelled_same_item"`. And this decision assumed
>    services were settled while
>    [`architecture-contract.md`](./architecture-contract.md) records them as
>    undecided; that conflict is resolved under §Scope of generation.
>
> **The two goals and the two mechanisms still stand.** What fell is one sentence —
> "token scanning recovers the contract" — and everything that had been built on top
> of it.
...
> does not is "token scanning recovers the contract". The shape was handed to #18,
> **which closed without taking it** — #42 / ADR-0014 picked it up, along with the
> other two judgements ADR-0009 orphaned there (the gate's value, and the `scope` /
> `kind` replacements).
...
| **Leave no lie** | A declared effect actually happens — this **would** be a lower bound | **Generation** (scanning `handle`'s tokens). Types cannot produce it, and neither does generation: #37 measured that token presence is neither a subset nor a superset of what runs, so [ADR-0014](../adr/0014-syntactically-present-replaces-observed.md) **abandoned the lower bound**. What is left is `syntactically_present`, plus the gap stated in `unverified_boundaries` |
...
```text
declared_ceiling       hand-written #[contract] + type enforcement
                       → "nothing else happens"          (an upper bound, enforced)
syntactically_present  generated by scanning one item's tokens
                       → "this is written here"          (NOT "this happens")

syntactically_present ⊄ declared     → compile error (the type half; already in place)
declared \ syntactically_present ≠ ∅ → over-declaration. CI warns (below)
```

> **`→ "this is written here"` is doing work in that diagram.** An earlier version
> read `→ "this happens"`, which is exactly the claim renaming the key was meant to
> remove — the rename had been applied to the key and not to the label beside it.
> Renaming a field does not rename the arrows pointing at it.
...
```json
"syntactically_present": { "fields": ["User::name"], "scope": "ctx_spelled_same_item", "deferred": "unknown" }
```

> **⚠️ `"ctx_spelled_same_item"` overstates what T-M1-07 measured, and is due for
...
#### The scan's scope and the architecture spec — resolved (#42, ADR-0014)

[`architecture-contract.md`](./architecture-contract.md) opens by constraining
"handler → service → repository", and this scan covers `handle` only. **Follow both
as written and the set is empty while `deferred` holds everything** — at which point
the gate above pushes `@service` onto every endpoint and the escape hatch becomes the
main road. That is the failure [`read-contract.md`](./read-contract.md) describes at
the point where it deleted `into_owned`.

**Resolved by narrowing the architecture claim, not by extending the scan.** The
First PoC's shape is the handler-only scan, and `handler → service → repository` is
**not** its recommended path. Two things already in the specs make this the smaller
move:

* `architecture-contract.md` §The service layer already records its own position as
  **undecided** — "no code example anywhere contains one", and `ctx.users()`
  "makes bypassing the service the shortest path".
* [`evaluation.md`](./evaluation.md) measured the 29%-shorter result **because the
  service layer disappeared.** The shape that measured well and the shape the
  architecture spec called correct were opposites, and only one of them was measured.

**Whether Verum has a service layer at all is still undecided.** What changes is
that this decision stops depending on the answer: the scan's scope is stated as one
item, and a service body is one case of `unscanned_effect` (path 22) rather than the
category the misses belong to — #37 measured that most of them are inside `handle`.

### When over-declaration appears — CI **warns** (was: fails)

> **⚠️ Revised by T-M1-07 (#37), decided by #42 /
> [ADR-0014](../adr/0014-syntactically-present-replaces-observed.md).** This section
> was argued on the assumption that the difference has no false positives. It has
> both directions: a missed effect produces a false **over**-declaration report, and
> a `#[cfg]`-gated statement produces a false **under**-declaration one. The sample
> error below asserts "declared but never mutated in `handle`", which the scan cannot
> establish, and neither suggested repair is correct for a renamed-parameter or
> aliased-receiver shape.
>
> **What the gate can and cannot see, now that it is measured:**
>
> | Case | Detected? |
> |---|---|
> | A declared effect **nowhere written** in the item — a stale contract, or one copied from another endpoint | **yes.** This is what the gate is for |
> | A declared effect written but **never executed** (`if false { … }`) | **no.** Dead code is in *both* sets — it must be declared (`E0277`, probe D2) and it is scanned (probe D1) — so the difference is empty |
> | An effect the scan missed (renamed `ctx`, aliased receiver, sibling item, `macro_rules!`) | reported as a **false** over-declaration |
> | A `#[cfg]`-gated effect on code never compiled | reported as a **false** under-declaration, whose repair is to **widen** the contract — the Q-C bias (RK-010) |
>
> **So the gate is a warning, not a failure, until cause B is closed.** That is a
> change from this section's original "CI fails": a gate that fails a *correct*
> endpoint because its parameter is named `cx` teaches people to delete the gate.
> The bypass half — an **undeclared** effect — is unaffected and stays a compile
> error.
...
warning is proportionate and a failure is not. An undeclared effect — a bypass —
